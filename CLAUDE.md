...
**Per-turn cost metadata:** every `/chat` row carries `latency_ms`, `prompt_tokens`, and `completion_tokens`. A turn bills up to three LLM calls (`contextualize_query` → answer → `get_best_link`); `chat_endpoint` sums them into one row, and branches that make no LLM call (greetings, canned deferrals) log zeros so every row has the same keys. `contextualize_query` and `get_best_link` therefore return `(value, usage)`. The faithfulness judge runs on a daemon thread spawned *after* that row is written, so its tokens go into its own `faithfulness_log.jsonl` record instead. `/generate-title` has no conversation-log row and is deliberately unattributed. `dashboard_data.compute_dashboard` aggregates the counts the same way it aggregates `latency_ms` — rows predating token accounting carry no keys and are filtered out, not read as zeros.
